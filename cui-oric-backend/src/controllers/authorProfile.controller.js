...
const updateMyProfile = catchAsync(async (req, res) => {
  ...
  if (updates.researchInterests) {
    ...
    const currentProfile = await AuthorProfile.findOne({ userId: req.user._id });
  ...
  if (updates.orcidId && !/^\d{4}-\d{4}-\d{4}-\d{3}[\dX]$/.test(updates.orcidId)) {
...
const uploadPhoto = catchAsync(async (req, res) => {
  ...
  const profile = await AuthorProfile.findOne({ userId: req.user._id }).select('+photoKey');
  if (!profile) {
    throw new NotFoundError('Author profile not found');
  }
...
const getAuthorProfile = catchAsync(async (req, res) => {
  ...
  if (profile.userId && profile.userId.status !== 'active') {
    if (!req.user || (req.user.role !== 'oric_admin' && req.user._id.toString() !== profile.userId._id.toString())) {
      throw new NotFoundError('Author profile not found');
    }
  }
...
const listAuthorProfiles = catchAsync(async (req, res) => {
  const { page = 1, limit = 20, departmentId, researchInterest, search, sortBy, sortOrder } = req.query;

  const query = {};

  if (departmentId) query.departmentId = departmentId;
  if (researchInterest) query.researchInterests = researchInterest.toLowerCase().trim();
  if (search) {
    query.$or = [
  ...
  const resolvedSortOrder = sortOrder && sortOrder.toLowerCase() === 'asc' ? 'asc' : 'desc';
  ...
  return success(res, profiles, 'Profiles retrieved', paginationMeta(page, limit, total));
});
...
const recomputeMetrics = catchAsync(async (req, res) => {
  const profileId = req.params.id === 'me'
    ? (await AuthorProfile.findOne({ userId: req.user._id }).select('_id'))?._id
    : req.params.id;
  ...
  const isOwner = profile.userId && profile.userId._id.toString() === req.user._id.toString();
  const isAdmin = req.user.role === 'oric_admin';

  if (!isOwner && !isAdmin) {
    throw new ForbiddenError('Not authorized to recompute metrics for this profile');
  }
...
const getMetricsSummary = catchAsync(async (req, res) => {
  ...
  // Recent publications
  const recentPubs = await Publication.find({ 'authors.authorId': profile.userId._id })
