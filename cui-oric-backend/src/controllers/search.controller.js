...
const searchPublications = catchAsync(async (req, res) => {
  ...
  if (researchInterest) {
    ...
    if (authorIds.length > 0) {
      ...
      return success(res, [], 'No publications found', paginationMeta(page, limit, 0));
    }
  }
  ...
  return success(res, publications, 'Search results', paginationMeta(page, limit, total));
});
...
const getSuggestions = catchAsync(async (req, res) => {
  ...
  const suggestions = { titles: [], authors: [], venues: [], keywords: [], researchInterests: [] };
  ...
  if (type === 'all' || type === 'titles') {
    const titleResults = await Publication.find(
      ...
      .limit(limit)
      .lean();
    suggestions.titles = [...new Set(titleResults.map((p) => p.title))].slice(0, limit);
  }

  // Author suggestions
  if (type === 'all' || type === 'authors') {
    const authorResults = await AuthorProfile.find({
      'userId': { $in: await User.find({ name: regex, status: 'active' }).select('_id').lean().then((u) => u.map((x) => x._id)) },
    })
      .populate('userId', 'name')
      .limit(limit)
      .lean();
    suggestions.authors = authorResults.map((p) => p.userId?.name).filter(Boolean).slice(0, limit);
  }
  ...
  if (type === 'all' || type === 'venues') {
    const venueResults = await Publication.find(
      ...
      .limit(limit)
      .lean();
    suggestions.venues = [...new Set(venueResults.map((p) => p.venue?.name))].filter(Boolean).slice(0, limit);
  }
  ...
  if (type === 'all' || type === 'researchInterests') {
    const interestResults = await ResearchInterest.find({ tag: regex, isActive: true })
...
const getFilters = catchAsync(async (req, res) => {
  ...
  const departments = await Publication.aggregate([
    ...
    { $unwind: '$dept' },
    { $project: { _id: 1, name: '$dept.name', campus: '$dept.campus', count: 1 } },
  ]);
  ...
  return success(res, { years, types, departments, interests }, 'Filters retrieved');
});
...
const advancedSearch = catchAsync(async (req, res) => {
  ...
  if (title) textConditions.push({ title: { $regex: escapeRegex(title), $options: 'i' } });
  if (venue) textConditions.push({ 'venue.name': { $regex: escapeRegex(venue), $options: 'i' } });
  if (keyword) textConditions.push({ keywords: { $regex: escapeRegex(keyword), $options: 'i' } });
  if (doi) textConditions.push({ doi: { $regex: escapeRegex(doi), $options: 'i' } });
  ...
  if (author) {
    ...
    if (authorIds.length > 0) {
      ...
      textConditions.push({ 'authors.externalName': { $regex: escapeRegex(author), $options: 'i' } });
    }
  }
  ...
  if (researchInterests && researchInterests.length > 0) {
    const profiles = await AuthorProfile.find({
      researchInterests: { $in: researchInterests.map((r) => r.toLowerCase().trim()) },
    }).select('userId');
    const authorIds = profiles.map((p) => p.userId);
    if (authorIds.length > 0) {
      query['authors.authorId'] = { $in: authorIds };
  ...
  return success(res, publications, 'Advanced search results', paginationMeta(page, limit, total));
});
